...
**Event**: [MakeUofT 2026](https://makeuoft.ca/) — Canada's largest hardware hackathon  
**Theme**: Doomsday Survival (D-Day)  
**Duration**: 24 hours  
**Team**: Collaborative effort during hackathon sprint  

### Inspiration

*"In a world inspired by Mad Max, survival isn't just about finding resources—it's about doing so without losing a limb. We wanted to build a tool for the post-apocalyptic scavenger: an AI-powered companion that handles the dirty work of identifying and retrieving vital scraps from the radioactive dust."*

### What It Does

Scrapple is an autonomous robotic arm controlled via a sleek, intuitive web interface:

1. **Identify**: Using a mounted camera and computer vision, Scrapple scans the environment and highlights "scraps" (useful objects) on your screen.
2. **Select**: The user simply clicks a scrap in the UI or speaks a voice command.
3. **Retrieve**: Scrapple's AI takes over, autonomously calculating the trajectory to pick up the item and safely deposit it into a collection bin.
...
- **The Vision**: YOLO v11 handles real-time object detection.
...
The "Wasteland" started in the terminal. We fought through:
- **Dependency Hell**: Package conflicts and version mismatches
- **Frontend/Backend Merge Conflicts**: Real-time integration challenges
- **Google Colab Timeouts**: Race against time to save training weights
- **Camera Resource Conflicts**: CV and LeRobot both needing camera access
- **API Key Security**: Brief scare involving pushed keys (quickly resolved)

### Accomplishments We're Proud Of

The **"Eureka" moment**: Sending a command from a browser button and watching a physical arm, built from scratch, successfully grasp an object. Seeing the YOLO v11 bounding boxes align perfectly with the robotic movement was our ultimate win. **It's officially ready for the doomsday.**
